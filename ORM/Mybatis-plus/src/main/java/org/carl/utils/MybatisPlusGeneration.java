  ...
  public static void main(String[] args) {
    ...
    FastAutoGenerator.create(db1.getJdbcUrl(), db1.getUsername(), db1.getPassword())
        ...
        .globalConfig(
            builder -> {
              builder
                  .author(Fields.AUTHOR) // 设置作者名
                  .outputDir(
                      System.getProperty("user.dir")
                          + "/ORM/Mybatis-plus/src/main/java") // 设置输出路径：项目的 java
              ...
              builder
                  ...
                  .pathInfo(
                      Collections.singletonMap(
                          OutputFile.xml,
                          System.getProperty("user.dir")
                              + "/ORM/Mybatis-plus/src/main/resources/mapper")); // 配置 mapper.xml
                                                                                 // 路径信息：项目的
                                                                                 // resources 目录下
            })
        // 4、策略配置
        .strategyConfig(
            builder -> {
              builder
                  ...
                  .columnNaming(
                      NamingStrategy
                          .underline_to_camel) // 数据库表字段映射到实体的命名策略：下划线转驼峰命。（默认是和naming一致，所以也可以不设置）
                  .addTableFills(
                      new Column("create_time", FieldFill.INSERT),
                      new Column(
                          "modify_time",
                          FieldFill
                              .INSERT_UPDATE)) // 添加表字段填充，"create_time"字段自动填充为插入时间，"modify_time"字段自动填充为插入修改时间
