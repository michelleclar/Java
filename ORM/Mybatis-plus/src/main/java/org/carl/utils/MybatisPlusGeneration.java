...
public class MybatisPlusGeneration {
    public static void main(String[] args) {
        DataSource db1 = DB.getMysqlDataSourceById("db1");


        //数据库表的设置
        List<String> listTable = List.of("user");  //设置需要自动代码生成的表名
        List<String> listTableSuffix = List.of("_b");    //设置 过滤 表的后缀
        ...
        //1、配置数据源
        FastAutoGenerator.create(db1.getJdbcUrl(), db1.getUsername(), db1.getPassword())
                //2、全局配置
                .globalConfig(builder -> {
                    builder.author(Fields.AUTHOR) // 设置作者名
                            .outputDir(System.getProperty("user.dir") + "/ORM/Mybatis-plus/src/main/java")   //设置输出路径：项目的 java 目录下【System.getProperty("user.dir")意思是获取到项目所在的绝对路径】
...
//                            .other("config")    //自定义包名(一般不在这里生成，而是后面编写的时候自己建包）
                            .pathInfo(Collections.singletonMap(OutputFile.xml, System.getProperty("user.dir") + "/ORM/Mybatis-plus/src/main/resources/mapper"));    //配置 mapper.xml 路径信息：项目的 resources 目录下
                })
                //4、策略配置
