...
public class JooqContextFactory {
  ...
  private Configuration getConfiguration(RequestContext requestContext) {
    Configuration configuration = new DefaultConfiguration().set(pgDataSource)
        .set(new Settings().withExecuteLogging(true).withRenderFormatted(true)
            .withRenderCatalog(false).withRenderSchema(false).withMaxRows(Integer.MAX_VALUE)
            .withRenderQuotedNames(RenderQuotedNames.EXPLICIT_DEFAULT_UNQUOTED)
            .withRenderNameCase(RenderNameCase.LOWER_IF_UNQUOTED).withExecuteLogging(true));
